...
export class StorefrontService {
  ...
  async getVehicle(slug: string, vehicleId: string) {
    const tenant = await this.resolveTenant(slug);
    const vehicle = await this.prisma.raw.vehicle.findUnique({
      where: { id: vehicleId },
      select: PUBLIC_VEHICLE_SELECT,
    });
    if (!vehicle) throw new NotFoundException('Vehicle not found');
    // findUnique above ignores tenant scope by design (raw client) -- verify ownership explicitly.
    const owned = await this.prisma.raw.vehicle.findFirst({
      where: { id: vehicleId, tenantId: tenant.id },
    });
    if (
      !owned ||
      !PUBLIC_VEHICLE_STATUSES.includes(
        owned.status as (typeof PUBLIC_VEHICLE_STATUSES)[number],
      )
    ) {
  ...
  async createInquiry(slug: string, dto: CreateInquiryDto) {
    const tenant = await this.resolveTenant(slug);
    if (dto.vehicleId) {
      const vehicle = await this.prisma.raw.vehicle.findFirst({
