...
export class StorefrontAuthGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    try {
      const payload = this.jwt.verify<CustomerTokenPayload>(authHeader.slice('Bearer '.length), {
        secret: this.config.get<string>('STOREFRONT_JWT_SECRET'),
      });
      if (payload.type !== 'storefront') throw new UnauthorizedException('Invalid token');
