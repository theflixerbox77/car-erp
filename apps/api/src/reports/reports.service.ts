...
export class ReportsService {
  ...
  async salesReport() {
    ...
    const byMonth = new Map(
      months.map((m) => [m, { month: m, count: 0, revenue: 0 }]),
    );
    ...
    return {
      ...
      totalRevenue: sales.reduce(
        (s, x) => s + toNum(x.salePrice) - toNum(x.discount),
        0,
      ),
    };
  }
  ...
  async expensesReport() {
    ...
    for (const e of expenses) {
      ...
      byCategory.set(
        e.category.name,
        (byCategory.get(e.category.name) ?? 0) + toNum(e.amount),
      );
    }

    return {
      trend: Array.from(byMonth.values()),
      byCategory: Array.from(byCategory.entries()).map(([name, total]) => ({
        name,
        total,
      })),
  ...
  async profitReport() {
    ...
    const months = lastNMonths(12);
    const byMonth = new Map(
      months.map((m) => [m, { month: m, revenue: 0, profit: 0 }]),
    );
  ...
  async inventoryReport() {
    const [byStatus, vehicles] = await Promise.all([
      ...
      this.prisma.client.vehicle.findMany({
        select: { brand: true, status: true, totalCost: true },
      }),
    ]);
    ...
    return {
      ...
      byBrand: Array.from(byBrand.entries()).map(([brand, count]) => ({
        brand,
        count,
      })),
  ...
  async customersReport() {
    ...
    const [customers, salesByCustomer] = await Promise.all([
      ...
      this.prisma.client.sale.groupBy({
        by: ['customerId'],
        _count: { _all: true },
        _sum: { salePrice: true },
      }),
    ...
    const months = lastNMonths(12);
    const byMonth = new Map(
      months.map((m) => [m, { month: m, newCustomers: 0 }]),
    );
  ...
  async leadsReport() {
    ...
    const completed =
      byStage.find((s) => s.stage === 'completed')?._count._all ?? 0;
  ...
  async stockAgingReport() {
    ...
    const items = vehicles.map((v) => {
      const ageDays = Math.floor(
        (now - v.createdAt.getTime()) / (1000 * 60 * 60 * 24),
      );
  ...
  async modelPerformanceReport() {
    const sales = await this.prisma.client.sale.findMany({
      ...
      select: {
        createdAt: true,
        vehicle: { select: { brand: true, model: true, createdAt: true } },
      },
    });

    const groups = new Map<
      string,
      { brand: string; model: string; totalDays: number; count: number }
    >();
    for (const s of sales) {
      ...
      const days = Math.max(
        0,
        Math.floor(
          (s.createdAt.getTime() - s.vehicle.createdAt.getTime()) /
            (1000 * 60 * 60 * 24),
        ),
      );
    ...
    const ranked = Array.from(groups.values())
      .map((g) => ({
        ...
        model: g.model,
        unitsSold: g.count,
        avgDaysToSell: Math.round(g.totalDays / g.count),
      }))
      .sort((a, b) => a.avgDaysToSell - b.avgDaysToSell);

    return {
      fastSelling: ranked.slice(0, 5),
      slowSelling: ranked.slice(-5).reverse(),
    };
  }
}
